...
このアプリはサンドボックス化されており、Core Audioのキャプチャ処理に必要なオーディオ入力のエンタイトルメントのみを使用します。ファイルへのアクセスは要求しません。

## プライバシー

音声はMac上でローカルに解析されます。キャプチャした音声を録音、保存、送信することはありません。
